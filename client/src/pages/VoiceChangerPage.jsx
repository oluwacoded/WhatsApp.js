...
export default function VoiceChangerPage({ standalone = false }) {
  ...
  useEffect(() => {
    navigator.mediaDevices.getUserMedia({ audio: true })
      ...
      .catch(() => setErr('Microphone blocked — tap the lock icon in your browser address bar, allow microphone, then refresh.'))
  }, [])
  ...
  const start = async () => {
    ...
    try {
      ...
      // Try with exact device ID first; fall back to default if the ID is stale
      let stream
      try {
        ...
        // "A value with the given key could not be found" = device ID stale → use default
        stream = await navigator.mediaDevices.getUserMedia({
      ...
      const shift = new Tone.PitchShift({ pitch: VOICE_PRESETS[preset].pitch, windowSize: 0.1, delayTime: 0, feedback: 0 })
      ...
      if (msg.toLowerCase().includes('permission') || msg.toLowerCase().includes('denied') || msg.toLowerCase().includes('not allowed')) {
        setErr('Microphone permission denied — tap the lock icon in the address bar, allow microphone, then refresh.')
      } else if (msg.toLowerCase().includes('key') || msg.toLowerCase().includes('notfound') || e.name === 'NotFoundError') {
        setErr('Microphone not found. Make sure you allowed mic access, then try again.')
      } else {
        setErr('Could not start: ' + msg + '. Try refreshing the page.')
      }
      setStatus('error')
    }
  }
  ...
  return (
    <div className={standalone ? 'min-h-screen bg-slate-950 p-4 md:p-8' : ''}>
      <div className="max-w-2xl mx-auto space-y-5">
            ...
            <div className="divide-y divide-blue-900/30">
              <div className="px-4 py-3 flex gap-3">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-semibold text-slate-100">Download free VB-Cable</p>
                  <p className="text-xs text-slate-400">This creates a fake virtual microphone on your PC. WhatsApp will use it and hear your changed voice.</p>
              ...
              <div className="px-4 py-3 flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">After restart, come back to this page in Chrome or Edge.</p>
                </div>
              </div>
              <div className="px-4 py-3 flex gap-3">
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-slate-800/60 rounded-lg p-3 space-y-1.5 text-xs">
                    <div className="flex justify-between">
                      <span className="text-slate-400">🎤 Your Real Mic</span>
                      <span className="text-slate-200 font-medium">your normal microphone</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">🔊 Send Voice To</span>
                      <span className="text-pink-300 font-medium">CABLE Input (VB-Audio)</span>
                    </div>
                  </div>
                  <p className="text-[11px] text-slate-500">No "CABLE Input" in the list? VB-Cable didn't finish — restart your PC again.</p>
                </div>
              </div>
              <div className="px-4 py-3 flex gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-100">Pick a voice, press Start</p>
                  <p className="text-xs text-slate-400">You should see the volume bar move when you speak. That means it's working.</p>
                </div>
              </div>
              <div className="px-4 py-3 flex gap-3">
                ...
                <div>
                  ...
                  <div className="bg-slate-800/60 rounded-lg p-3 mt-1 text-xs text-slate-300">
                    <p><span className="text-blue-300 font-medium">WhatsApp Desktop</span>: Settings → Devices → Microphone → select <span className="text-pink-300 font-semibold">"CABLE Output (VB-Audio)"</span></p>
                  </div>
                </div>
              </div>
              <div className="px-4 py-3 flex gap-3">
        ...
        {platform === 'android' && (
          <div className="bg-purple-950/40 border border-purple-800/40 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-purple-800/30">
              ...
              <p className="text-[11px] text-purple-400/70 mt-0.5">Android has 2 easy ways to do this — pick one below.</p>
            </div>

            {/* Method 1 — Speaker trick */}
            <div className="px-4 py-4 border-b border-purple-900/30">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-2 flex-1">
                  ...
                  <div className="bg-slate-800/60 rounded-xl p-3 space-y-2.5 text-xs">
                    <div className="flex gap-2.5 items-start">
                      <span className="w-5 h-5 shrink-0 rounded-full bg-slate-700 text-slate-300 font-bold flex items-center justify-center text-[10px]">1</span>
                      <span className="text-slate-200">Start the WhatsApp call on <span className="font-semibold text-white">Phone 1</span>, put it on <span className="font-semibold text-emerald-300">speaker mode</span></span>
                    </div>
                    <div className="flex gap-2.5 items-start">
                      <span className="w-5 h-5 shrink-0 rounded-full bg-slate-700 text-slate-300 font-bold flex items-center justify-center text-[10px]">2</span>
                      <span className="text-slate-200">On <span className="font-semibold text-white">Phone 2</span> (this page), pick your voice below and press <span className="font-semibold text-emerald-300">Start</span></span>
                    </div>
                    <div className="flex gap-2.5 items-start">
                      <span className="w-5 h-5 shrink-0 rounded-full bg-slate-700 text-slate-300 font-bold flex items-center justify-center text-[10px]">3</span>
                      <span className="text-slate-200">Hold Phone 2 close to Phone 1, speak into Phone 2</span>
                    </div>
                    <div className="flex gap-2.5 items-start">
                      <span className="w-5 h-5 shrink-0 rounded-full bg-emerald-700 text-emerald-200 font-bold flex items-center justify-center text-[10px]">✓</span>
                      <span className="text-emerald-300 font-medium">The other person hears your changed voice from Phone 2's speaker via Phone 1's mic</span>
                    </div>
                  </div>
                  <p className="text-[11px] text-slate-500">Works with any Android. No apps to install. No rooting needed.</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-4 py-4 border-b border-purple-900/30">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-400">If you have a PC or laptop, call from WhatsApp Web there. Switch to the <span className="text-white font-medium">PC tab</span> above for full instructions. Best quality and zero lag.</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-4 py-3">
              <p className="text-[11px] text-slate-500">
                <span className="text-slate-400 font-medium">Why can't I route directly?</span> Android blocks web pages from sending audio straight into WhatsApp's mic for security. No rooting needed for Methods 1 or 2 — only Method 1 needs a second phone.
              </p>
            </div>
          </div>
        )}

        {/* Device selectors — hide Send Voice To on mobile (setSinkId not supported) */}
        {!isRunning && (
            ...
            <div>
              ...
              <div className="relative">
                <select value={selectedInput} onChange={e => setSelectedInput(e.target.value)}
                  ...
                  <option value="">Default (your phone mic)</option>
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-slate-400 flex items-center gap-1.5">
                <Volume2 size={12} />
                {muted ? 'Muted — nothing going out' : 'Live — speak now to test'}
              </span>
            ...
            {onMobile && (
              <p className="text-[11px] text-emerald-400 text-center mt-2 font-medium">
                🔊 Voice is changing! Put this phone's speaker next to your other phone's mic
              </p>
            )}
          </div>
        )}
        ...
        {/* Error */}
        {err && (
          <div className="bg-red-900/20 border border-red-700/40 rounded-lg px-4 py-3 text-xs text-red-400">
            ❌ {err}
          </div>
        )}
