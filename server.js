...
settings.paymentsEnabled = !!process.env.FLUTTERWAVE_SECRET || !!process.env.PAYSTACK_SECRET;
    ...
    for (const msg of messages) {
      ...
      if (text.startsWith(pfx)) {
        ...
        if (cmd === "createacct" || cmd === "pay") {
          const flwSecret = process.env.FLUTTERWAVE_SECRET;
          if (!flwSecret) {
            await send("⚠️ payments are not configured on this backend.\nAdd FLUTTERWAVE_SECRET in Railway variables, then redeploy.");
            continue;
          }
